...
void deconvLocalDouble(NVMatrix& images, NVMatrix& target,
  const int numFilters, const int imgSize, const int outputsX);
void deconvLocalDoubleUndo(NVMatrix& grads, NVMatrix& target,
  const int numFilters, const int imgSize, const int outputsX);
...
void convLocalPool(NVMatrix& images, NVMatrix& target, int numFilters,
    ...
    getLastCudaError("convLocalPool: kernel execution failed, before entering");
    int numImages = images.getNumCols();
