...
void convResizeBilinear(NVMatrix& images, NVMatrix& target, int imgSize, int tgtSize, float scale) {
    ...
    if (imgsPerThread == 4) {
        if (checkCaseBounds) {
            ...
            kResizeBilinear<4, true><<<blocks, threads, 0, stream>>>(images.getDevData(), target.getDevData(), imgSize, tgtSize, numImages, images.getStride(), scale, (float)centerScale);
        } else {
            cudaFuncSetCacheConfig(kResizeBilinear<4, false>, cudaFuncCachePreferL1);
            kResizeBilinear<4, false><<<blocks, threads, 0, stream>>>(images.getDevData(), target.getDevData(), imgSize, tgtSize, numImages, images.getStride(), scale, (float)centerScale);
        }
    } else if (imgsPerThread == 2) {
        if (checkCaseBounds) {
            cudaFuncSetCacheConfig(kResizeBilinear<2, true>, cudaFuncCachePreferL1);
            kResizeBilinear<2, true><<<blocks, threads, 0, stream>>>(images.getDevData(), target.getDevData(), imgSize, tgtSize, numImages, images.getStride(), scale, (float)centerScale);
        } else {
            cudaFuncSetCacheConfig(kResizeBilinear<2, false>, cudaFuncCachePreferL1);
            kResizeBilinear<2, false><<<blocks, threads, 0, stream>>>(images.getDevData(), target.getDevData(), imgSize, tgtSize, numImages, images.getStride(), scale, (float)centerScale);
        }
    } else {
        if (checkCaseBounds) {
            cudaFuncSetCacheConfig(kResizeBilinear<1, true>, cudaFuncCachePreferL1);
            kResizeBilinear<1, true><<<blocks, threads, 0, stream>>>(images.getDevData(), target.getDevData(), imgSize, tgtSize, numImages, images.getStride(), scale, (float)centerScale);
        } else {
            cudaFuncSetCacheConfig(kResizeBilinear<1, false>, cudaFuncCachePreferL1);
            kResizeBilinear<1, false><<<blocks, threads, 0, stream>>>(images.getDevData(), target.getDevData(), imgSize, tgtSize, numImages, images.getStride(), scale, (float)centerScale);
        }
    }
 ...
 * deconvLocalDouble
 */
template<int imgsPerThread>
__global__ void kLocalDouble(float* imgs, float* target, const int numFilters, const int imgSize, const int outputsX, const int numCases) {
  const int idx = (blockIdx.x * blockDim.x + threadIdx.x) * imgsPerThread;
  const int stride = gridDim.x * blockDim.x * imgsPerThread;
  for (uint i = idx; i < numFilters * imgSize * imgSize * numCases; i += stride) {
	const uint n0 = i % numCases;
	const uint w0 = (i / numCases) % imgSize;
	const uint h0 = (i / (numCases * imgSize)) % imgSize;
	const uint c0 = i / (numCases * imgSize * imgSize);
	const uint imgIdx0 = ((c0 * imgSize + h0) * imgSize + w0) * numCases + n0;
	const uint tgtIdx0 = ((c0 * outputsX + h0 * 2) * outputsX + w0 * 2) * numCases + n0;
	const uint tgtIdx1 = ((c0 * outputsX + h0 * 2) * outputsX + w0 * 2 + 1) * numCases + n0;
	const uint tgtIdx2 = ((c0 * outputsX + h0 * 2 + 1) * outputsX + w0 * 2) * numCases + n0;
	const uint tgtIdx3 = ((c0 * outputsX + h0 * 2 + 1) * outputsX + w0 * 2 + 1) * numCases + n0;
#pragma unroll
	for (int j = 0; j < imgsPerThread; ++j) {
	  target[tgtIdx0 + j] = imgs[imgIdx0 + j];
	  target[tgtIdx1 + j] = imgs[imgIdx0 + j];
	  target[tgtIdx2 + j] = imgs[imgIdx0 + j];
	  target[tgtIdx3 + j] = imgs[imgIdx0 + j];
	}
  }
}
 ...
 */
void deconvLocalDouble(NVMatrix& images, NVMatrix& target, const int numFilters, const int imgSize, const int outputsX) {
  int numImages = images.getNumCols();
  ...
  assert(target.isContiguous());
  int imgsPerThread = numImages % 128 == 0 ? 8 : (numImages % 64 == 0 ? 4 : (numImages % 32 == 0 ? 2 : 1));
  const int kBlocks = 4096;
  const int kThreadsPerBlock = 512;
  cudaStream_t stream = NVMatrix::getDefaultStream();
//	printf("threads: %dx%d, blocks: %dx%d, imgSize: %d, numFilters: %d, numImages: %d outputsX: %d\n",
//	  threads.y, threads.x, blocks.y, blocks.x, imgSize, numFilters, numImages, outputsX);
  if (imgsPerThread == 8) {
	cudaFuncSetCacheConfig(kLocalDouble<8>, cudaFuncCachePreferL1);
	kLocalDouble<8> << <kBlocks, kThreadsPerBlock, 0, stream >> >(images.getDevData(), target.getDevData(),
		numFilters, imgSize, outputsX, numImages);
  } else if (imgsPerThread == 4) {
	cudaFuncSetCacheConfig(kLocalDouble<4>, cudaFuncCachePreferL1);
	kLocalDouble<4> << <kBlocks, kThreadsPerBlock, 0, stream >> >(images.getDevData(), target.getDevData(),
		numFilters, imgSize, outputsX, numImages);
  } else if (imgsPerThread == 2) {
	cudaFuncSetCacheConfig(kLocalDouble<2>, cudaFuncCachePreferL1);
	kLocalDouble<2> << <kBlocks, kThreadsPerBlock, 0, stream >> >(images.getDevData(), target.getDevData(),
	  numFilters, imgSize, outputsX, numImages);
  } else if (imgsPerThread == 1) {
	cudaFuncSetCacheConfig(kLocalDouble<1>, cudaFuncCachePreferL1);
	kLocalDouble<1> << <kBlocks, kThreadsPerBlock, 0, stream >> >(images.getDevData(), target.getDevData(),
	  numFilters, imgSize, outputsX, numImages);
  }
  getLastCudaError("deconvLocalDouble: kernel execution failed");
}

/*
 * deconvLocalDoubleUndo
 */
template<int imgsPerThread>
__global__ void kLocalDoubleUndo(float* grads, float* target, const int numFilters, const int imgSize, const int outputsX, const int numCases) {
  const int idx = (blockIdx.x * blockDim.x + threadIdx.x) * imgsPerThread;
  const int stride = gridDim.x * blockDim.x * imgsPerThread;
  for (uint i = idx; i < numFilters * imgSize * imgSize * numCases; i += stride) {
	const uint n0 = i % numCases;
	...
	const uint h0 = (i / (numCases * imgSize)) % imgSize;
	const uint c0 = i / (numCases * imgSize * imgSize);
	const uint tgtIdx0 = ((c0 * imgSize + h0) * imgSize + w0) * numCases + n0;
	const uint gradsIdx0 = ((c0 * outputsX + h0 * 2) * outputsX + w0 * 2) * numCases + n0;
	const uint gradsIdx1 = ((c0 * outputsX + h0 * 2) * outputsX + w0 * 2 + 1) * numCases + n0;
	const uint gradsIdx2 = ((c0 * outputsX + h0 * 2 + 1) * outputsX + w0 * 2) * numCases + n0;
	const uint gradsIdx3 = ((c0 * outputsX + h0 * 2 + 1) * outputsX + w0 * 2 + 1) * numCases + n0;
#pragma unroll
	for (int j = 0; j < imgsPerThread; ++j) {
	  target[tgtIdx0 + j] = grads[gradsIdx0 + j] +
		grads[gradsIdx1 + j] +
		grads[gradsIdx2 + j] +
		grads[gradsIdx3 + j];
	}
...
/*
 * grads:		(numFilters, outputsX, numImages)
 * target:		(numFilters, imgSize, numImages)
 */
void deconvLocalDoubleUndo(NVMatrix& grads, NVMatrix& target, const int numFilters, const int imgSize, const int outputsX) {
  int numImages = grads.getNumCols();

  int outputs = outputsX * outputsX;
  int imgPixels = imgSize * imgSize;
  assert(grads.getNumRows() == numFilters * outputs);
  ...
  //	  threads.y, threads.x, blocks.y, blocks.x, imgSize, numFilters, numImages, outputsX);
  if (imgsPerThread == 8) {
	cudaFuncSetCacheConfig(kLocalDoubleUndo<8>, cudaFuncCachePreferL1);
	kLocalDoubleUndo<8> << <kBlocks, kThreadsPerBlock, 0, stream >> >(grads.getDevData(), target.getDevData(),
	  numFilters, imgSize, outputsX, numImages);
  } else if (imgsPerThread == 4) {
	cudaFuncSetCacheConfig(kLocalDoubleUndo<4>, cudaFuncCachePreferL1);
	kLocalDoubleUndo<4> << <kBlocks, kThreadsPerBlock, 0, stream >> >(grads.getDevData(), target.getDevData(),
	  numFilters, imgSize, outputsX, numImages);
  } else if (imgsPerThread == 2) {
	cudaFuncSetCacheConfig(kLocalDoubleUndo<2>, cudaFuncCachePreferL1);
	kLocalDoubleUndo<2> << <kBlocks, kThreadsPerBlock, 0, stream >> >(grads.getDevData(), target.getDevData(),
	  numFilters, imgSize, outputsX, numImages);
  } else if (imgsPerThread == 1) {
	cudaFuncSetCacheConfig(kLocalDoubleUndo<1>, cudaFuncCachePreferL1);
	kLocalDoubleUndo<1> << <kBlocks, kThreadsPerBlock, 0, stream >> >(grads.getDevData(), target.getDevData(),
	  numFilters, imgSize, outputsX, numImages);
  }
  getLastCudaError("deconvLocalDoubleUndo: kernel execution failed");
}

/*
 * fpropActsPRelu
 */
__global__ void kFpropActsPRelu(float* inputs, float* target, float* weights, const int numWeights, const int numCols) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const int stride = gridDim.x * blockDim.x;
  for (int i = idx; i < numWeights * numCols; i += stride) {
	const float inputVal = inputs[i];
	target[i] = inputVal > 0.0f ? inputVal : weights[i / numCols] * inputVal;
  }
}

void fpropActsPRelu(NVMatrix& inputs, NVMatrix& target, NVMatrix& weights, const int numWeights) {
  int numCols = inputs.getNumElements() / numWeights;
  assert(!inputs.isTrans());
  assert(inputs.isContiguous());
  assert(weights.isContiguous());
  assert(weights.getNumElements() == numWeights);
  ...
  cudaStream_t stream = NVMatrix::getDefaultStream();
  cudaFuncSetCacheConfig(kFpropActsPRelu, cudaFuncCachePreferL1);
  kFpropActsPRelu<<<kBlocks, kThreads, 0, stream>>>(inputs.getDevData(), target.getDevData(), weights.getDevData(), numWeights, numCols);

  getLastCudaError("fpropActsPRelu: kernel execution failed");
}

/*
 * bpropActsPRelu
 */
__global__ void kBpropActsPRelu(float* grads, float* target, float* weights, float* inputs, const int numWeights, const int numCols) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const int stride = gridDim.x * blockDim.x;
  for (int i = idx; i < numWeights * numCols; i += stride) {
	target[i] = inputs[i] > 0.0f ? grads[i] : grads[i] * weights[i / numCols];
  }
}

void bpropActsPRelu(NVMatrix& grads, NVMatrix& target, NVMatrix& weights, NVMatrix& inputs, const int numWeights) {
  int numCols = grads.getNumElements() / numWeights;
  assert(!grads.isTrans());
  assert(grads.isContiguous());
  assert(!inputs.isTrans());
  assert(inputs.isContiguous());
  ...
  const int kThreads = 512;
  cudaStream_t stream = NVMatrix::getDefaultStream();
  cudaFuncSetCacheConfig(kBpropActsPRelu, cudaFuncCachePreferL1);
  kBpropActsPRelu<<<kBlocks, kThreads, 0, stream>>>(grads.getDevData(), target.getDevData(), weights.getDevData(), inputs.getDevData(), numWeights, numCols);

  getLastCudaError("bpropActsPRelu: kernel execution failed");
}