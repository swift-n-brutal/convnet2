...
void NVMatrix::batchedMatrixMultiply(NVMatrixV& a, NVMatrixV& b, NVMatrixV& target, float scaleTarget, float scaleAB, cudaStream_t stream) {
    ...
    if (batch > 0) {
		...
		std::vector<const float*> aPtrsVec(batch), bPtrsVec(batch), tgtPtrsVec(batch);
		const float** aPtrs = aPtrsVec.data(), **bPtrs = bPtrsVec.data(), **tgtPtrs = tgtPtrsVec.data();
...
//        int height = getFollowingDim();
        ...
        if (width > 1) {
			...
			NVMatrix Tmp = target.getClone();
			this->apply(uop, Tmp, stream);
			target.applyBinary(bop, *this, target, stream);
