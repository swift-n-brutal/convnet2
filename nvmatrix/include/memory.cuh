...
#define HOST_ALLOC_CHUNK                    ((unsigned long long)1UL << 32)
...
// hack by ldp: from linux to windows, the clz
#ifdef _MSC_VER
#include <Windows.h>
#include <intrin.h>
int __inline __builtin_ctz( unsigned int value )
{
...
int __inline __builtin_clz( unsigned int value )
{
...
int __inline __builtin_ffs( unsigned int value )
{
	if(value == 0)
		return 0;
...
public:
    ...
    virtual MemoryManager* init() {
        ...
        int bucket = (int)GET_DEALLOC_BUCKET((unsigned int)_size);
    ...
    MemorySegment* malloc(size_t size) {
        assert(size > 0);
        int requestedBucket = (int)GET_ALLOC_BUCKET((unsigned int)size);
        _lock.acquire();
...
//        }
        ...
        if (bucket - requestedBucket > BUCKET_DIVISION_THRESHOLD) { // We got a much bigger chunk than we wanted
            ...
            int newSrcBucket = (int)GET_DEALLOC_BUCKET((unsigned int)sourceSegment->getSize());
    ...
    void free(MemorySegment* mem) {
        ...
        int bucket = (int)GET_DEALLOC_BUCKET((unsigned int)mem->getSize());
