...
class NeuronLayerParser(LayerWithInputParser):
    ...
    def parse_neuron(self, neuron_str):
        for neup in neuron_parsers:
            p = neup.parse(neuron_str)
...
class ChannelwiseAvgLayerParser(LayerWithInputParser):
    def __init__(self):
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        if len(dic['numInputs']) != len(dic['channels']):
            raise LayerParsingError("Layer '%s': inputs and channels must have the same length. Got : %d, %d" % (name, len(dic['numInputs']), len(dic['channels'])))
        img_pixels = [(dic['numInputs'][i] / dic['channels'][i]) for i in range(len(dic['numInputs']))]
        if len(set(img_pixels)) != 1:
            raise LayerParsingError("Layer '%s': all inputs must have the same dimensionality. Got dimensionalities: %s" % (name, ", ".join(str(s) for s in img_pixels)))
        dic['outputs'] = img_pixels[0]
        dic['usesInputs'] = False
        dic['usesActs'] = True
        
        print "Initialized channelwise avg layer '%s' on GPUs %s, producing %d outputs" % (name, dic['gpus'], dic['outputs'])
        return dic
...
#TODO        dic['initW'] = mcp.safe_get_float_list(name, 'initW')
...
#        print "flag 1", self.dic['initWFunc']
...
class DeconvLocalDoubleParser(LayerWithInputParser):
    def __init__(self):
        LayerWithInputParser.__init__(self, num_inputs=1)
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['outputsX'] = dic['imgSize'] * 2
        dic['outputs'] = dic['outputsX']**2 * dic['channels']
        
        dic['usesInputs'] = True
        dic['usesActs'] = True
        
        print "Initialized deconv local double layer '%s' on GPUs %s, producing %dx%d %d-channel output" % (name, dic['gpus'], dic['outputsX'], dic['outputsX'], dic['channels'])
        return dic

class PReluLayerParser(WeightLayerParser):
    def __init__(self):
        WeightLayerParser.__init__(self)
    ...
    def parse(self, name, mcp, prev_layers, model):
        ...
        dic['sharedParam'] = mcp.safe_get_bool(name, 'sharedParam', default=True)
        dic['channels'] = mcp.safe_get_int(name, 'channels', default=1)
        dic['outputs'] = dic['numInputs'][0]
        dic['weightsPerFilter'] = 1
        self.verify_num_range(dic['outputs'], 'outputs', 1, None)
        self.make_weights_const(dic['initW'], [1 if dic['sharedParam'] else dic['channels']], [1], order='C')
        self.make_biases(1, 1, order='C')
        dic['useActs'] = False

        print "Initialized parametric ReLU layer '%s' on GPUs %s, producing %d outputs" % (name, dic['gpus'], dic['outputs'])
        return dic
    
    def make_weights_const(self, initW, rows, cols, order='C'):
        dic = self.dic
        dic['weights'], dic['weightsInc'] = [], []
        if dic['initWFunc']: # Initialize weights from user-supplied python function
            # Initialization function is supplied in the format
            ...
            for i in xrange(len(dic['inputs'])):
                if dic['weightSourceLayers'][i] != '': # Shared weight matrix
                    ...
                    dic['weights'] += [initW[i] * n.ones((rows[i], cols[i]), order=order, dtype=n.single)]
                    dic['weightsInc'] += [n.zeros_like(dic['weights'][i])]
...
layer_parsers = {'data' :           lambda : DataLayerParser(),
                 ...
                 'chnavg':          lambda : ChannelwiseAvgLayerParser(),
                 ...
                 'deconv.double':   lambda : DeconvLocalDoubleParser(),
                 'prelu':           lambda : PReluLayerParser(),
...
neuron_parsers = sorted([NeuronParser('ident', 'f(x) = x', uses_acts=False, uses_inputs=False),
                         ...
                         ParamNeuronParser('prelu[a]', 'f(x) = max(x, 0) + a * min(x, 0)', uses_acts=False, uses_inputs=True)],
                        key=lambda x:x.type)
