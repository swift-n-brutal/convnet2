...
import math as m
import layer as lay
from convdata import ImageDataProvider, CIFARDataProvider, \
    DummyConvNetLogRegDataProvider, NoisyDataProvider, \
    OttoDataProvider
...
class ConvNet(IGPUModel):
    def __init__(self, op, load_dic, dp_params={}):
        filename_options = []
        for v in ('color_noise', 'multiview_test', 'inner_size', 'out_size', 'scalar_mean', 'minibatch_size', 'sigma', 'sigma_base'):
...
#                print self.layers[errname]['children']
    ...
    def conditional_save(self):
        ...
        if len(self.test_outputs) == 1 or (self.test_outputs[-1][0].values()[0][0] < self.best_test_error):
            self.best_test_error = self.test_outputs[-1][0].values()[0][0]
            self.save_state()
        else:
            print "Worse than %f, not saving." % self.best_test_error
            print "=======================================================",
    ...
    def get_options_parser(cls):
        ...
        op.add_option("out-size", "out_size", IntegerOptionParser, "Output patch size", default=0, set_once=True)
        ...
        op.add_option("sigma-base", "sigma_base", FloatOptionParser, "Noise level which the data are normalized to", default=25.0)
        ...
        DataProvider.register_data_provider('noisy', 'Gaussian noisy data provider', NoisyDataProvider)
        DataProvider.register_data_provider('otto', 'Otto data provider', OttoDataProvider)
