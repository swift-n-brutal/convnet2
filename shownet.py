...
class ShowConvNet(ConvNet):
    ...
    def init_data_providers(self):
        self.need_gpu = self.op.get_value('show_preds') or self.op.get_value('show_restored')
        class Dummy:
    ...
    def init_model_state(self):
        ...
        elif self.op.get_value('show_restored'):
            self.output_name = self.op.get_value('show_restored')
...
#            filters = filters / n.sqrt(n.tile(filters.reshape((filters.shape[0] * filters.shape[1], filters.shape[2])).var(axis=0).reshape(1, 1, filters.shape[2]), (filters.shape[0], filters.shape[1], 1)))
    ...
    def plottable_data(self, img, sizeY, sizeX):
        img_uint8 = img*255
...
#        preds = self.test_data_provider.get_plottable_data(preds.T)
        ...
        if self.save_preds:
            ...
            gs = gridspec.GridSpec(NUM_ROWS, NUM_COLS*NUM_PATS,
                                   width_ratios=[1]*NUM_COLS*NUM_PATS, height_ratios=[1]*NUM_ROWS )
            #print data[1]
            for row in xrange(NUM_ROWS):
                for col in xrange(NUM_COLS):
                    ...
                    pl.yticks([])
                    img = data[0][:,img_idx]
                    pl.imshow(self.plottable_data(img, patch_size, patch_size), interpolation='nearest')
                    ...
                    pl.imshow(self.plottable_data(r_img, patch_size, patch_size), interpolation='nearest')
                    ...
                    nc_img = n_img.reshape(noisy_size, noisy_size)[border:-border, border:-border]
                    ...
                    pl.imshow(self.plottable_data(n_img, noisy_size), interpolation='nearest')
...
#        print self.show_cost
        ...
        if self.show_restored:
            self.plot_restored_img()
    ...
    def get_options_parser(cls):
        ...
        op.add_option("show-restored", "show_restored", StringOptionParser, "Show restored images made by given output on test set", default="")
        ...
        op.add_option("sigma", "sigma", ListOptionParser(IntegerOptionParser), 'Noise level for noisy layer', default=[25])
        op.add_option("image-folder", "image_folder", StringOptionParser, "Folder of images to restore", default="", requires=['show_restored'])
        op.add_option("image-names", "image_names", StringOptionParser, "File of image names", default="", requires=['image_folder'])
