...
# Initialize weight matrix using by Otto model
def makew_orsm(name, idx, shape, params):
    fh = open(params[0], 'r')
    checkpoint_info = cPickle.load(fh)
    fh.close()
    w_vh = checkpoint_info['w_vh']
    w_h = checkpoint_info['w_h']
    assert(w_vh.shape[0] == shape[0])
    assert(w_vh.shape[1] == shape[1])
    assert(w_h.shape[0] == shape[1])
    return (w_vh + w_h).astype(n.float32)
    
# Initialize weight matrix of FC using Xavier
def makew_fc(name, idx, shape, params):
    pass