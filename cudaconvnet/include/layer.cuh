...
class ChannelwiseAvgLayer : public Layer {
protected:
  // floatv* _coeffs;
  intv* _channelsV;
  int _total_channels;
  ...
  void truncBwdActs();
public:
  ChannelwiseAvgLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
  ~ChannelwiseAvgLayer();
};

class DeconvLocalDoubleLayer : public Layer, public TwoDLayerInterface {
protected:
  void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
  void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
  DeconvLocalDoubleLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicID);
};

class PReluLayer : public WeightLayer {
protected:
  int _channels;
  NVMatrix _weightGradTmp;
  virtual void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
...
public:
  ...
  PReluLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool useGrad);
  virtual bool updateWeights();
};
