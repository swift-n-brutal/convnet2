 ...
 * -----------------------
 *
 * f(x) = max(0, x) + a * min(x, 0)
 * =======================
 */
class ParamReluNeuron : public Neuron {
protected:
  float _a;

  virtual void _activate() {
	_inputs->apply(ParamReluOperator(_a), *_outputs);
  }

  void _computeInputGrad(NVMatrix& actsGrad, NVMatrix& target) {
	actsGrad.applyBinary(ParamReluGradientOperator(_a), *_inputs, target);
  }

  void _addInputGrad(NVMatrix& actsGrad, NVMatrix& target) {
	actsGrad.applyTernary(AddGradientBinaryOperator<ParamReluGradientOperator>(ParamReluGradientOperator(_a)), *_inputs, target, target);
  }
public:
  class ParamReluOperator {
  private:
	float _a;
  public:
	ParamReluOperator(float a) : _a(a) {}
	__device__ inline float operator()(float x) const {
	  return x > 0.0f ? x : _a * x;
	}
  };

  class ParamReluGradientOperator {
  private:
	float _a;
  public:
	ParamReluGradientOperator(float a) : _a(a) {}
	__device__ inline float operator()(float unitActGrad, float unitInput) const {
	  return unitInput > 0.0f ? unitActGrad : _a*unitActGrad;
	}
  };

  ParamReluNeuron(float a) : _a(a), Neuron() {}
};
