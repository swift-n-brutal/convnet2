...
//    _outputs = _actsTarget < 0 ? new NVMatrix() : &_prev[_actsTarget]->getActs();
    if (!_initialized) {
        ...
        if (getReplicaIdx() == 0 && _convNetThread != NULL) {
            ...
            _actBroadcaster = new ActBroadcaster(getNumSiblingReplicas());
 ...
 * =======================
 */
ChannelwiseAvgLayer::ChannelwiseAvgLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID) : Layer(convNetThread, paramsDict, replicaID, false) {
  //_coeffs = pyDictGetFloatV(paramsDict, "coeffs");
  _channelsV = pyDictGetIntV(paramsDict, "channels");
  _total_channels = 0;
  for (vector<int>::const_iterator it = _channelsV->begin(); it != _channelsV->end(); it++) {
	_total_channels += (*it);
  }
  assert(_total_channels > 0);
}

ChannelwiseAvgLayer::~ChannelwiseAvgLayer() {
  delete _channelsV;
}

void ChannelwiseAvgLayer::fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx) {
  // add each channel together
  int channels = _channelsV->at(inpIdx);
  int num_cases = _inputs[inpIdx]->getNumCols();
  _inputs[inpIdx]->reshape(channels, _inputs[inpIdx]->getNumElements() / channels);
  getActs().reshape(1, getActs().getNumElements());
  getActs().addSum(*_inputs[inpIdx], 0, scaleTargets, 1.0 / _total_channels);
  getActs().reshape(getActs().getNumElements() / num_cases, num_cases);
  _inputs[inpIdx]->reshape(_inputs[inpIdx]->getNumElements() / num_cases, num_cases);
}

void ChannelwiseAvgLayer::bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType) {
  // _prev[replicaIdx][inpIdx]->getActsGrad().add(v, scaleTargets, 1.0 / _total_channels);
  int channels = _channelsV->at(inpIdx);
  v.tile(channels, 1, _gradTmp);
  // printf("bprop %d %d\n", _gradTmp.getNumRows(), _gradTmp.getNumCols());
  _prev[replicaIdx][inpIdx]->getActsGrad().add(_gradTmp, scaleTargets, 1.0 / _total_channels);
}

void ChannelwiseAvgLayer::truncBwdActs() {
  Layer::truncBwdActs();
  _gradTmp.truncate();
}
 ...
 * =======================
 */
DataCopyThread::DataCopyThread(DataLayer& parent, intv& cpus) : _parent(&parent), _sleepUsec(0), 
	//Thread(true, cpus) 
	Thread(true) 
{
}
...
//  printf("SumOfSquareCostLayer: %s entered. (%d) %d x %d\n", _name.c_str(), inpIdx, _inputs[inpIdx]->getNumRows(), _inputs[inpIdx]->getNumCols());
    _inputs[0]->apply(NVMatrixOps::Square(), getActs());
//	printf("SumOfSquareCostLayer: middle. (%d) %d x %d\n", inpIdx, getActs().getNumRows(), getActs().getNumCols());
    _costv.clear();
    _costv.push_back(getActs().sum());
//	printf("SumOfSquareCostLayer: exited. (%d) %d x %d\n", inpIdx, getActs().getNumRows(), getActs().getNumCols());
}
...
/*
 * ======================
 * DeconvLocalDoubleLayer
 * ======================
 */
DeconvLocalDoubleLayer::DeconvLocalDoubleLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID)
: Layer(convNetThread, paramsDict, replicaID, false), TwoDLayerInterface(paramsDict) {}

void DeconvLocalDoubleLayer::fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx) {
  deconvLocalDouble(*_inputs[inpIdx], getActs(), _channels, _imgSize, _imgSize * 2);
  /*int numCases = _inputs[inpIdx]->getNumCols();
  _inputs[inpIdx]->tile(1, 2, _mtxTmp);
  _mtxTmp.reshape(_channels * _imgSize, numCases * _imgSize * 2);
  _mtxTmp.tile(1, 2, getActs());
  getActs().reshape(_channels * _imgPixels * 4, numCases);
  _mtxTmp.truncate();*/
}

void DeconvLocalDoubleLayer::bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType) {
  // TODO : handle the cases of scaleTargets != 0, i.e. multi-passes
  deconvLocalDoubleUndo(v, _prev[replicaIdx][inpIdx]->getActsGrad(), _channels, _imgSize, _imgSize * 2);
}
 ...
 * =====================
 */
PReluLayer::PReluLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool useGrad) 
: WeightLayer(convNetThread, paramsDict, replicaID, false, useGrad) {
  _wStep = 0.01;
  _bStep = 0.01;
  int sharedParam = pyDictGetInt(paramsDict, "sharedParam");
  if (sharedParam == 0) {
	_channels = pyDictGetInt(paramsDict, "channels");
  } else {
	_channels = 1;
...
void PReluLayer::fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx) {
  fpropActsPRelu(*_inputs[inpIdx], getActs(), getWeightMatrix(passType, inpIdx), _channels);
}

void PReluLayer::bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType) {
  // TODO : handle the cases of scaleTargets != 0, i.e. multi-passes
  bpropActsPRelu(v, _prev[replicaIdx][inpIdx]->getActsGrad(), getWeightMatrix(passType, inpIdx), *_inputs[inpIdx], _channels);
}

void PReluLayer::bpropBiases(NVMatrix& v, PASS_TYPE passType) {}

void PReluLayer::bpropWeights(NVMatrix& v, int replicaIdx, int inpIdx, PASS_TYPE passType) {
  assert(_weights->at(inpIdx).isUseGrad());
  v.applyBinary(PReluMultiply(), *_inputs[inpIdx], _weightGradTmp);
  _weightGradTmp.reshape(_channels, _weightGradTmp.getNumElements() / _channels);
  float scaleGrad = getGradScale(inpIdx, passType);
  float scaleInc = getIncScale(inpIdx, passType);
  getGradTarget(inpIdx).addSum(_weightGradTmp, 1, scaleInc, scaleGrad);
}

bool PReluLayer::updateWeights() {
  if (getConvNet().getTotalPassesDone() % _weightUpdatePassPeriod == 0) {
	_weights->update(getConvNet().getTrainingProgress());
	_biases->update(getConvNet().getTrainingProgress());
	//        constrainWeights();
...
void PReluLayer::truncBwdActs() {
  Layer::truncBwdActs();
  _weightGradTmp.truncate();
}

void PReluLayer::_constrainWeights() {
  for (int i = 0; i < _weights->getSize(); ++i) {
	//float minw = _weights->at(i).getW().min();
	//float maxw = _weights->at(i).getW().max();
	//printf("%s : [%f, %f]\n", this->getName().c_str(), minw, maxw);
  }
}