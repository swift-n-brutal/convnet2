...
ConvNetThread::ConvNetThread(PyObjectV* layerList, int deviceID, int deviceIdx, ConvNet* convNet)
  //  : Thread(true, getDeviceCPUs(deviceID)), _deviceID(deviceID), _convNet(convNet) {
    : Thread(true), _deviceID(deviceID), _convNet(convNet) {
    try {
...
void ConvNetThread::initLayer(PyObject* paramsDict, int replicaID) {
    ...
    if (type == "fc") {
	    ...
	    _nameLayerMap[name] = new ChannelwiseAvgLayer(this, paramsDict, replicaID);
	} else if (type == "eltsum") {
	...
	} else if (type == "deconv.double") {
	  _nameLayerMap[name] = new DeconvLocalDoubleLayer(this, paramsDict, replicaID);
	} else if (type == "prelu") {
	  _nameLayerMap[name] = new PReluLayer(this, paramsDict, replicaID, true);
